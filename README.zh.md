...
**MiniBrain 是一个通过 MCP 提供能力的跨模型记忆连接器。**
...
MiniBrain 本质上是同一个产品、同一套记忆核心，只是提供两种部署方式：

- **本地模式（PC 单机）**：通过 stdio 运行，适合个人使用。
- **远程模式（网络版）**：通过 HTTP/SSE 运行，适合多设备或多 Agent 协作。

两种模式共享同一套记忆模型、检索逻辑和 MCP 工具接口。  
区别在传输层，不在产品本体。

| 你的需求 | 建议模式 |
|---|---|
| 只在一台电脑上快速使用 | **本地模式（`serve`）** |
| 需要跨设备/多人共享记忆 | **远程模式（`serve-remote`）** |
| 希望部署最简单 | **本地模式** |
| 需要公网访问/API Key 鉴权 | **远程模式** |
